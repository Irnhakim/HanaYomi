({
    getPopularManga: function(page) {
        var baseUrl = "https://api.komiku.org";
        ...
        while ((match = regex.exec(html)) !== null) {
            ...
            var slug = mUrl;
            if (slug.indexOf("komiku.org") !== -1) {
                slug = slug.substring(slug.indexOf("komiku.org") + 10);
            }
    ...
    getMangaDetails: function(mangaId) {
        ...
        var html = http.httpGet(baseUrl + path);
        if (!html) return {};
        ...
        var descMatch = html.match(/<p[^>]+class="desc"[^>]*>([\s\S]*?)<\/p>/) || html.match(/<p[^>]+itemprop="description"[^>]*>([\s\S]*?)<\/p>/) || html.match(/<div class="desc">([\s\S]*?)<\/div>/);
    ...
    getChapterList: function(mangaId) {
        ...
        var chapters = [];
        var regex = /<a href="([^"]+)" itemprop="url"[^>]*><span itemprop="name"[^>]*>([\s\S]*?)<\/span>/g;
        var match;
        while ((match = regex.exec(html)) !== null) {
            var chUrl = match[1].trim();
            var chName = match[2].replace(/<[^>]*>/g, "").trim();
